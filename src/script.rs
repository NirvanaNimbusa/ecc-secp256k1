    ...
    fn next(&mut self) -> Option<Instruction<'a>> {
        ...
        match opcodes::All::from(self.data[0]).classify() {
            opcodes::Class::PushBytes(n) => {
                ...
                if self.enforce_minimal && n == 1 && (self.data[1] == 0x81 || (self.data[1] > 0 && self.data[1] <= 16)) {
                    self.data = &[];
...
impl Builder {
    ...
    /// Pushes a public key
    pub fn push_key(self, key: &PublicKey, compressed: bool) -> Builder {
        if compressed{
            self.push_slice(&key.compressed()[..])
        } else {
            self.push_slice(&key.uncompressed()[..])
        }
    }
...
mod test {
    ...
    use crate::opcodes;
    use crate::secp256k1::PublicKey;
    use crate::utility::hex_to_bytes;
    ...
    fn script_init() {
        ...
        script = script.push_int(10_000_000); comp.extend([4u8, 128, 150, 152, 0].iter().cloned()); assert_eq!(&script[..], &comp[..]);
        script = script.push_int(-10_000_000); comp.extend([4u8, 128, 150, 152, 128].iter().cloned()); assert_eq!(&script[..], &comp[..]);

        // data
        script = script.push_slice("NRA4VR".as_bytes()); comp.extend([6u8, 78, 82, 65, 52, 86, 82].iter().cloned()); assert_eq!(&script[..], &comp[..]);

        // keys
        let keystr = "02eeeea7d79f3ecde08d2a3c59f40eb3adcac9defb77d3b92053e5df95165139cd";
        let key = PublicKey::from_compressed(&hex_to_bytes(keystr)[..]).unwrap();
        script = script.push_key(&key, true); comp.extend([vec!(33u8), hex_to_bytes(keystr)].concat().iter().cloned()); assert_eq!(&script[..], &comp[..]);
        let keystr = "046C5D5E73124F3C821C0985DF787E11B3D018A86ADD577FA8661613A0D49DDE59BE34F8EC266C26F94F94497E5B5D1F4159F6C0D2AF22ED5AB5BA41D19535499C";
        let key = PublicKey::from_uncompressed(&hex_to_bytes(keystr)[..]).unwrap();
        script = script.push_key(&key, false); comp.extend([vec!(65u8), hex_to_bytes(keystr)].concat().iter().cloned()); assert_eq!(&script[..], &comp[..]);
    ...
    fn script_builder() {
        ...
        let script = Builder::new().push_opcode(opcodes::all::OP_DUP)
                                   ...
                                   .push_slice(&hex_to_bytes("16e1ae70ff0fa102905d4af297f6912bda6cce19"))
   ...
   /* Script serialization is disabled for now
   requires consensus encoding and decoding rules
    #[test]
    fn script_serialize() {
